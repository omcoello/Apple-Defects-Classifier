  ...
  try:
    ...
    if len(intersection) > 0:
      for i in range(1, len(intersection)):
        ...
        #Spot defect restricting areas greater than 700
        area = masks[i]["area"]
        if area > 700:
...
rgbImagesPath = "images/cont/rgb"
nirImagesPath = "images/cont/nir"
...
for label in rgbImagesLabels:
  ...
  rgbFileName = "rgbSil_" + str(counter).zfill(4) + ".png"
  nirFileName = "nirSil_" + str(counter).zfill(4) + ".png"
  ...
  try:
    cv2.imwrite("images/segmentation/cont/rgb/"+ rgbFileName,resultRgb)

  except:
    err = open("fruitSilhouetteError.txt","a")
    err.write("Write Error: " + label + "\n")
    valInt += 1
  if valInt == 0:
    try:
      cv2.imwrite("images/segmentation/cont/nir/" + nirFileName,resultNir) 
      ...
      valInt += 2
  if valInt == 2:    
    os.remove("images/segmentation/cont/rgb/" + rgbFileName)
