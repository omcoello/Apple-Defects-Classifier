...
directorio_guardado_modelos = './siames_models/rgb-nir/MobileNetV1'
...
def generate_image_pairs(generator1, generator2):
    while True:
        ...
        yield ([x1, x2], y1)  # Corregir para que las etiquetas sean [y1, y2]

# Crear modelo MobileNetV1 preentrenado
mobilenet_base = MobileNet(weights='imagenet', include_top=False, input_shape=(960, 830, 3))

# Congelar las capas convolucionales preentrenadas
for layer in mobilenet_base.layers:
    layer.trainable = False

# Construir el modelo Siamese con MobileNetV1
...
shared_branch = models.Sequential([
    ...
    layers.Dense(128, activation='relu')  # Reducir el tamaño de la capa densa
], name='shared_branch')
...
ruta_guardado_modelo = os.path.join(directorio_guardado_modelos, 'epochs/siamese_MobileNetV1_epoca_RGB_NIR{epoch:02d}.h5')

# Callback para TensorBoard
log_dir = "logs_siames/fit/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + " MobileNetV1_RGB_NIR"
...
checkpoint = ModelCheckpoint(
    ...
    monitor='val_accuracy',  # Puedes cambiar a 'val_loss' o 'val_categorical_accuracy'
